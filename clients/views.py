...
def client_index(request):
    ...
    nb_producteurs = 0
    Total_plant = 0
    Total_plant = DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative_id__in=cooperatives).aggregate(total=Sum('nb_plante'))['total']
    ...
    for cooperative in cooperatives :
        ...
        # if DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative_id=cooperative).aggregate(total=Sum('nb_plante'))['total'] != None :
           # Total_plant = Total_plant + DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative_id=cooperative).aggregate(total=Sum('plant_recus'))['total']
    ...
    if len(cooperatives) > 1 :
        for coop in cooperatives:
            ...
            coop.plants = DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative=coop).aggregate(total=Sum('nb_plante'))['total']
        ...
        cooperatives.plants = DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative__utilisateur=request.user.utilisateur).aggregate(total=Sum('nb_plante'))['total']
        cooperatives.superficies = Parcelle.objects.filter(producteur__cooperative__utilisateur=request.user.utilisateur).aggregate(total=Sum('superficie'))[ 'total']
...
#     cooperatives = Cooperative.objects.filter(utilisateur = request.user.utilisateur)
#     for cooperative in cooperatives :
#         query = DetailPlanting.objects.filter(planting__parcelle__producteur__cooperative_id=cooperative).values("espece__libelle").annotate(nb_plante=Sum('nb_plante'))
#         for q in query :
#             querysets.append(q)
