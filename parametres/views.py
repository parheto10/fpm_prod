...
def map_parcelles(request):
    parcelles = Parcelle.objects.filter(producteur__cooperative__utilisateur=request.user.utilisateur).order_by('code')
...
@api_view(['GET'])
def show_monitoring(request, code=None):
    # cooperative = Cooperative.objects.get(user_id=request.user.id)
    monitorings = Monitoring.objects.filter(planting_id=code)
...
@api_view(['GET'])
def map_plantings_espece(request, code=None):
    groups = []
    user = request.user
    if user.is_authenticated:
        groups = list(user.groups.values_list('name', flat=True))

    queryset = DetailPlanting.objects.filter(planting_id=code)
    planting = Planting.objects.get(code=code)
